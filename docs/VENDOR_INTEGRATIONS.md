...
**Vendoring status (2026-08-24):** the full Owen pantheon is vendored into this repository at pinned commits under `packages/vendor/upstream/` — `poseidon` (671053b), `gaia` (f23a32d), `dryad` (e85729b), `demiurge` (cb3e178), `tiamat` (e389a99, Tier B reference — license pending the BOM read), `apate` (ae62cac, reference), `minos` (ea8f76e, reference). The four Tier A wraps exist as real workspace packages — `@3jse/water-poseidon`, `@3jse/foliage-gaia`, `@3jse/flora-dryad`, `@3jse/terrain-demiurge` — each re-exporting the pure core now (generation math, no Three.js import) with the rendering half as the editor-slice step, exactly as described below. Registry entries carry `pinnedCommit` + `vendored` fields; `tools/license-scan.mjs` records every vendored LICENSE in the notebook.
