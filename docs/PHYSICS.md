...
Babylon.js runs four physics backends (Ammo, Cannon, Havok, Oimo) behind one plugin interface in production, which is useful here as validation, not as a reason to widen scope: it confirms a real, shipping engine has proven that "one interface, swappable WASM backend" holds up under actual gameplay-code load rather than just looking clean on paper, and its plugin interface is the concrete reference for what `@3jse/physics`'s own surface should cover (body/collider CRUD, raycasts, joints, character-controller primitives) so `@3jse/physics-jolt` isn't designed in a vacuum when it's actually built. It is not, on its own, a reason to ship more than the two backends above list — a third WASM physics engine is only worth adding when a real project need shows up, the same bar `VENDOR_INTEGRATIONS.md` applies everywhere else.
