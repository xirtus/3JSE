...
**The model behind PLAN.** `@3jse/llm` is a provider-agnostic chat client (OpenAI, Anthropic, DeepSeek, Gemini, OpenRouter, Groq, Mistral, xAI, Ollama, or any OpenAI-compatible endpoint — two wire protocols, `openai-chat` and `anthropic-messages`, cover all of them). The editor's **AI Providers** panel is where a user picks one and pastes an API key; the key lives in that browser's `localStorage` only and is sent straight to the chosen provider, nothing in between. Once a provider is active, Atlas's "Ask agent" button and the Agent panel's "Plan with AI" send the scoped-context package (`@3jse/atlas`'s `exportAgentContext` + `previewChange`) to the model and surface its plan for a human to act on. The model proposes; it does **not** auto-execute tool calls — ACT stays human-driven until trust tiers (below) are built.
