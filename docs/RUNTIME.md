...
**Proven in the reference games** (`REFERENCE_GAMES.md`): MANDELHOP builds its whole difficulty model on this posture — constant-velocity lanes, seeded per-run generation, render-only visuals, and collision on grid math *on the same numbers the player sees*, so every death is fair and replays reproduce; its time powers all route through one clock (`worldDt = dt · scale`), and its 12 Hz ghost replay is recorded state, nothing more. DAMN BEAVERS extends the same posture to counterfactual replay — fork/alter/diff over snapshots. The runtime's fixed-step accumulator with a time-scale is exactly the mechanism those games hand-build.

`@3jse/replay` (`ROADMAP.md` Phase 5) is this claim exercised for real, not just asserted: it records the exact `InputManager` press/release edges and the tick they happened on, then replays them against a freshly-built, identical starting `World`. Its own test suite proved this holds precisely down to a real, load-bearing subtlety — replaying without calling `InputManager.endFrame()` on every tick (this doc's tick loop stage 1) leaves edge-triggered reads like `wasActionPressed("jump")` true for extra ticks, producing a fully reproducible *wrong* result that looks exactly like non-determinism until traced back to the missing call. With that contract actually followed, per-machine determinism holds precisely through `@3jse/physics-rapier`'s Rapier/WASM simulation too, not just plain gameplay Systems — confirmed by test, not assumed from this paragraph's own claim.
...
**This mode is already proven in production, not speculative** (`REFERENCE_GAMES.md`): 3JSURF's `mechcheck.mjs` runs wave + surfer with no renderer/canvas/DOM and asserts its numbers match the rendered build; ZENDRIVE's `npm test` runs full races headless; DAMN BEAVERS' simulation tests run the entire systemic sim headless — its constitution states it outright: *"the renderer, animation system, UI, audio, particles, and Chronicle observe simulation state; they do not own simulation truth."* The discipline those projects impose by convention (gameplay logic must be renderer-independent; a stub scene must suffice) is exactly what the runtime enforces by construction here.
