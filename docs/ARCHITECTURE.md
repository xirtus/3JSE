...
3. **Everything the editor can do, code can do.** Every editor mutation — moving an entity, adding a component, wiring a graph node — has a corresponding call in the runtime's command API. The editor is a GUI for that API, not a separate authority. This is what makes the Agent API (`AI_AGENT_API.md`) possible without inventing a second interface. The community Babylon.js Editor is the cautionary example this principle exists to rule out: a capable GUI built *on top of* a code-first engine that was never designed to expose an equivalent command API, so the editor and hand-written code inevitably drift into two only-partially-overlapping authorities over the same scene. 3JSE's editor panels don't get a privileged path — Physics, 3JSE Graph, the Debugger, and the Agent Panel all read and write the same `World`/`Level`/`IRGraph` objects a hand-written script would, through the identical calls, which is what keeps this from becoming that failure mode by construction rather than by discipline.
