...
| # | Task | Status | Evidence | Commit |
|---|---|---|---|---|
| H.0 | `node 3JSE_Harness_v0.2/scripts/verify-harness.mjs` green | done (ongoing) | PASS 2026-09-01 (with new stricter checks) | (uncommitted) |
| H.2 | Harness v0.2 — guard rails adopted from vibe web-starter-kit (MIT): destructive-git PreToolUse guard (wired live in repo `.claude/settings.json`), clean-code PostToolUse ratchet + baseline (opt-in), `wip/` workspace, agent operating rules in `AGENTS.md` (commit-early-on-branch, no-wholesale-delete, bench-is-not-evidence, lessons+falsified findings, comment policy, no-MD-via-shell, channel compression, frames-prove-not-debug, lab-not-playable-scene, headed-only visual QA), QA skill updates, folder renamed `3JSE_Harness_v0.1` → `v0.2`. `typescript` added to root devDeps for the clean-code tooling. | done | `pnpm verify:harness` PASS + guard smoke cases | (uncommitted) |
...
| A.4 | Atlas v0.2+ — extra lenses, runtime pulses, time scrubber, A/B FeelSpec, Feel Lab, `atlas/` manifest, live LLM planning, 2.5D/Three.js layer | **done (for this scope)** — all §5 lenses shipped: event/perf/provider/asset/stateMachine/gameplayFlow + **world (§5.10) / style (§5.8) / trace (§5.5) / rig (§5.11)**; `TraceRecorder` + `pulseCounts`; A/B FeelSpec (§16); `parseAtlasManifest` (§44); editor **Trace time scrubber** (play/pause rAF sweep + seek slider + `◉` pulse on nodes fired <0.3s ago + density strip). atlas suite 48. Deliberately deferred: Feel Lab (§17), live LLM planning (needs an LLM), 2.5D/Three.js layer (§63). | `packages/atlas/src/{lenses,feelAB,manifest,world,style,trace,rig}.ts`; `apps/editor/src/panels/AtlasPanel.tsx`; `docs/ATLAS.md` | `9acf95c`, `198eeeb` |
| A.5 | Website promotion — Atlas + AI Harness as the two flagship products on 3jse.com | done — new `#flagships` section (2 product cards) right after the pitch, promoted in nav + hero; manual bakes `ATLAS` + `3JSE_ATLAS_FULL_PLAN` + `HARNESS` + `REFERENCE_GAMES` + `ENGINE_GAP_ANALYSIS` (29→32 docs, 0 broken xrefs); builder auto-links baked docs only. **Pending: SFTP upload to 3jse.com host (index.html + manual.html) + HTTPS cert.** | `site/{index.html,manual.html,build-manual.mjs}` | `f4fb020`, `4123590` |

---

## Gap roadmap  (`docs/ENGINE_GAP_ANALYSIS.md` §6 — the net-new content-production subsystems)

Each: a headless-first `@3jse/*` core with tests + (mostly) an editor panel. Trailing work is
depth + the GPU/viewport/service half — see the "Left:" note per §6 item.

| # | Task | Status | Evidence | Commit |
|---|---|---|---|---|
| G.1 | **Packaging** (`BUILD_DEPLOYMENT.md`) — `@3jse/packaging`: `planBuild` (tree-shake / asset finalize / LOD drop / FNV hash / code-split / manifest+buildId), `checkPublishGate` (fails on unattributed staged imports), `publish` (manifest + NOTICES + index.html + a bootstrap that boots a project headless of the editor; pwa adds sw/webmanifest). Editor Packaging panel. | done (core) — 11 tests | `packages/packaging` | 537b27a |
| G.2 | **Audio** (`AUDIO.md`) — `@3jse/audio`: `MixerGraph` (bus tree + ducking), AudioSource/Listener/ReverbZone + `createAudioSystem` (pluggable `AudioBackend`, NullBackend for tests), `AudioEventRouter`, musical grid (`quantize` / `scaleDegreeToMidi`) + `MusicDirector` (24-PPQN MIDI clock) + `MidiOut`/`NullMidiOut`. | done (core) — 8 tests | `packages/audio` | 8453cc9 |
| G.3 | **UI / HUD** — `@3jse/ui`: retained `UINode` tree, `computeLayout` (flexbox subset), `resolveTree` (bind to `resource:`/`entity:` paths → text/fill/visible/color), `hitTest`, `UIRenderer` seam + `NullRenderer`, `HUDManager`. | done (core) — 6 tests | `packages/ui` | 258093d |
| G.4 | **Animation retargeting** — `@3jse/animation` gains `retargetClip` (bone rename + rest-delta rotation compensation + hip-height translation scale), `autoMapSkeleton` (canonicalized names). Editor Animation panel (state machine view + live retarget demo). | done (core) — +7 tests (25) | `packages/animation/src/retarget.ts` | 259a67f, 682596f |
| G.5 | **Material Graph → TSL** — `@3jse/materials`: `MaterialGraph` model, `validateGraph` (cycle/arity/no-color), `compileToTSL` (deterministic three/tsl codegen), `evaluateGraph` (CPU reference evaluator). Editor Material Graph panel (validation + TSL + CPU preview strip), verified in Chrome. | **done** — 9 tests | `packages/materials` | 68dc9e9, 682596f |
| G.6 | **Terrain + Foliage runtime** — `@3jse/terrain` (`valueNoise2D`/`fbm`, `meshChunk`, `lodResolution`, `TerrainStreamer` bounded residency) + `@3jse/foliage` (`scatterArea` seeded jittered-grid with slope/height/exclusion/spline constraints, `toInstanceMatrices`). Editor Terrain panel (residency + thumbnail). | done (core) — 9 + 7 tests | `packages/{terrain,foliage}` | c92410b, 682596f |
| G.7 | **Navigation** — `@3jse/nav`: `bakeNavGrid`, `findPath` (octile A* + string-pull), `buildFlowField` (group pathing), `NavAgent` + `createNavAgentSystem`. | done (core) — 8 tests | `packages/nav` | f86ff97 |
| G.8 | **Networking transport + priority** — `@3jse/networking` gains `PriorityAccumulator` (interest cull + distance falloff + starvation + byte budget), `HistoryBuffer` (rewind + `validateHit` ray test), `WebSocketTransport`. | done (core) — +7 tests (14) | `packages/networking/src/{priority,lagcomp,websocket}.ts` | b73f9fb |
| G.9 | **Particles / VFX** — `@3jse/vfx`: `sampleCurve`/`sampleGradient`, `ParticlePool` (SoA, seeded, cone spread, gravity/drag, swap-remove, cap), `ParticleEmitter` + `createParticleSystem`. Editor Particles panel (live rAF sim on a canvas), verified in Chrome. | **done** — 7 tests | `packages/vfx` | 2fa64c1, 682596f |

### Gap roadmap — depth pass (the "Left:" half; reuse-first)

| # | Task | Status | Evidence | Commit |
|---|---|---|---|---|
| G.10 | **Viewport rendering** for terrain/foliage/particles — new `@3jse/render` (imports `three/webgpu`): `TerrainRenderer` (chunk BufferGeometry from `meshChunk` + streamer delta), `FoliageRenderer` (`scatterArea` → InstancedMesh), `ParticleRenderer` (`pool.buffers()` → Points). `Terrain`/`FoliageField` authoring components. Editor `Viewport.tsx` builds them from the components. **Verified in Chrome: rolling terrain + scattered grass render live in the viewport.** | done — 3 tests; no new external dep | `packages/render`; `apps/editor/src/{Viewport,sampleScene,vfxScene}.ts` | d6973ad |
| G.11 | **`3jse` CLI** — `@3jse/cli`: `3jse publish [dir] --target --tier --out` runs `@3jse/packaging.publish()` + writes output; defers JS bundling to esbuild (dynamic optional import; emits a `build.mjs` when absent). `3jse info` lists the catalog. | done — 5 tests | `packages/cli` | 184306f |
| G.12 | **Web Audio backend** — `@3jse/audio` `WebAudioBackend`: same `AudioBackend` interface; source→gain (2D) / source→PannerNode(HRTF)→gain (spatial); per-clip buffer cache + in-flight dedupe; setGain via `setTargetAtTime`; setListener. | done — +4 tests (12); `@3jse/audio` → shipped | `packages/audio/src/webaudio.ts` | 75f4d31 |
| G.13 | **Polygon navmesh seam** — `@3jse/nav` `PolyNavMesh` interface + `createRecastNavMesh(query)` adapting **recast-navigation-js** (isaac-mason, MIT, optional peer) + `gridAsPolyNavMesh` fallback. Provenance in `packages/vendor/licenses.json`. | done — +5 tests (13); `@3jse/nav` → shipped | `packages/nav/src/polymesh.ts` | 0d02226 |
| G.14 | **Reusable node canvas** — `@3jse/graph` `NodeCanvas`: model-agnostic pan / zoom / drag / select / bezier edges. Editor Atlas System Map renders through it (dragged positions override auto-layout). | done — +3 tests (9) | `packages/graph/src/NodeCanvas.tsx` | d69b565 |
| G.15 | **Atlas v0.2 remaining lenses** — `stateMachineLens` (§5.3), `gameplayFlowLens` (§5.2). Live LLM planning behind "Ask agent" is deliberately not faked — the §28 context export + §30 preview are real; planning waits on an actual LLM (same posture as AI_AGENT_API.md's PLAN stage). | **done** — +2 tests (43). Trace/Style/World/Rig lenses + scrubber followed in `9acf95c`/`198eeeb` (see A.4). LLM planning still the one deliberate gap. | `packages/atlas/src/lenses.ts` | 55bb071 |

### Install + polish tail  (`RESUME.md` "What's left", 2026-09-02)

| # | Task | Status | Evidence | Commit |
|---|---|---|---|---|
| G.16 | **`3jse` CLI bundles for real** — `esbuild` (`^0.24.2`, MIT) is now a real dep of `@3jse/cli`; `runPublish`'s bundle callback runs `esbuild.build` (stdin entry, ESM, minify, `three*` external) instead of deferring to a hand-run `build.mjs`. | done — 5 tests still green | `packages/cli/src/bin.ts`; `packages/vendor/licenses.json` | `86f885f`, `bd36355` |
| G.17 | **Polygon navmesh bake** — new `@3jse/nav-recast`: `bakeRecastNavMesh(meshes, config?)` runs **recast-navigation-js** `threeToSoloNavMesh` + `NavMeshQuery` and returns a `@3jse/nav` `PolyNavMesh` via `createRecastNavMesh`; `toRecastQuery` adapter + `collectWalkable`. Kept separate so `@3jse/nav` stays WASM-free/three-free. | done — 6 tests incl. a real WASM bake across a ground plane | `packages/nav-recast/` | `86f885f` |
| G.18 | **GPU particle render path** — `@3jse/render` `GpuParticleRenderer`: drop-in for `ParticleRenderer` (same `sync(pools)` call site) using `PointsNodeMaterial` — per-particle size + soft sprite in a TSL node graph, streams into `StorageBufferAttribute`s. `@3jse/vfx` stays the headless sim authority; full GPU-compute re-sim deferred by design. Editor Viewport uses it. | done — +1 test (4); reuse-first (three-vfx rejected — R3F-coupled, owns its own sim) | `packages/render/src/GpuParticleRenderer.ts`; `apps/editor/src/Viewport.tsx` | `198eeeb` |
| G.19 | **TSL splat terrain material + paint tools** — `@3jse/terrain` `splat.ts` (`createSplatMap` / `paintSplat` brush / `sampleSplat` / `splatToTexture`), `@3jse/materials` `presets.ts` (`splatTerrainGraph` / `waterGraph`), `@3jse/render` `terrainSplatMaterial` (RGBA `DataTexture` + TSL channel-mix). Fixed a latent `edge.fromPin` (`.r/.g/.b/.a`) bug in **both** the CPU evaluator and the TSL codegen. Editor terrain renders slope/height-seeded splat. | done — +14 tests (terrain 15, materials 13, render 4) | `packages/{terrain,materials,render}/src/*`; `apps/editor/src/Viewport.tsx` | `4aeac50` |
| G.20 | **In-app LLM providers — the model behind PLAN** — new `@3jse/llm` (provider-agnostic chat client: OpenAI / Anthropic / DeepSeek / Gemini / OpenRouter / Groq / Mistral / xAI / Ollama / any OpenAI-compatible; two wire protocols `openai-chat` + `anthropic-messages`; `fetch` injected for tests). Editor **AI Providers** panel — pick a provider, paste a key, pick a model, Test; keys in `localStorage` only, sent straight to the provider. Atlas "Ask agent" + Agent panel "Plan with AI" now call the configured model and print the plan (ACT still human-driven). | done — +11 tests (@3jse/llm 6, editor store 5) | `packages/llm/`; `apps/editor/src/llm/{store,plan}.ts`; `apps/editor/src/panels/{AiProvidersPanel,AtlasPanel,AgentPanel}.tsx` | (this commit) |
